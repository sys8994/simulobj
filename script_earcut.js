...
// --- START: Earcut 라이브러리 코드를 여기에 직접 삽입 ---
var earcut = function (vertices, holeIndices, dim) {

    dim = dim || 2;

    var hasHoles = holeIndices && holeIndices.length,
        outerLen = hasHoles ? holeIndices[0] * dim : vertices.length,
...
function linkedList(vertices, start, end, dim, clockwise) {
    var i, last;
    ...
    do {
        ...
        while (b !== a.prev) {
            if (a.i !== b.i && isValidDiagonal(a, b)) {
                ...
                earcutLinked(b2, triangles, dim, minX, minY, invSize);
                return;
            }
            b = b.next;
        }
...
function eliminateHoles(vertices, holeIndices, outerNode, dim) {
    var queue = [],
        i, len, start, end, list;

    for (i = 0, len = holeIndices.length; i < len; i++) {
        start = holeIndices[i] * dim;
        end = i < len - 1 ? holeIndices[i + 1] * dim : vertices.length;
...
function findHoleBridge(hole, outerNode) {
    ...
    do {
        if (hx >= p.x && p.x >= mx && hx !== p.x &&
                pointInTriangle(my < hy ? hx : qx, hy, mx, my, my < hy ? qx : hx, hy, p.x, p.y)) {

            tan = Math.abs(hy - p.y) / (hx - p.x);

            if (isvalid(tan) && (tan < tanMin || (tan === tanMin && (p.x > m.x || (p.x === m.x && sectorContainsSector(m, p)))))) {
                m = p;
                tanMin = tan;
            }
        }
...
function isValidDiagonal(a, b) {
    return a.next.i !== b.i && a.prev.i !== b.i && !intersectsPolygon(a, b) &&
            ...
            equals(a, b) && area(a.prev, a, b.prev) > 0 && area(b.prev, b, a.next) > 0;
}
...
earcut.deviation = function (vertices, holeIndices, dim, triangles) {
    var hasHoles = holeIndices && holeIndices.length;
    var outerLen = hasHoles ? holeIndices[0] * dim : vertices.length;

    var polygonArea = Math.abs(signedArea(vertices, 0, outerLen, dim));
    if (hasHoles) {
        for (var i = 0, len = holeIndices.length; i < len; i++) {
            var start = holeIndices[i] * dim;
            var end = i < len - 1 ? holeIndices[i + 1] * dim : vertices.length;
